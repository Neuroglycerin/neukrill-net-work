...
def predict(cache_paths, out_fname, clf, settings, train_split=0.8):
    ...
    X_train = joblib.load(cache_path[0])
    X_test = joblib.load(cache_path[1])
...
clf = sklearn.ensemble.RandomForestClassifier(n_estimators=2000, max_depth=30, min_samples_leaf=3, n_jobs=16, random_state=42)

for pathpair in cache_paths:
    print pathpair
    out_fname = pathpair[0][:-4] + '_predictions.csv'
